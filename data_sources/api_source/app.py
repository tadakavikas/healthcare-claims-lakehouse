"""Source 2: enrollment events REST API (simulates a SaaS / streaming-style source).

Behaves like a real event API:
  * Cursor-based pagination (`next_cursor`)
  * Incremental pulls via `?since=<ISO timestamp>` on event_time
  * ~5% of events are late: event_time is hours older than created_at
    (this is why the ingestion layer needs a watermark + lookback window)

Endpoints:
    GET /health
    GET /api/v1/enrollment-events?since=2026-07-01T00:00:00&limit=100&cursor=0
"""
...
# Deterministic in-memory event store, regenerated on startup.
# Real APIs are backed by a DB; a seeded list keeps the demo reproducible.
...
def _build_events(days: int = 30, per_hour: int = 40) -> None:
    ...
    for h in range(days * 24):
        ...
        for _ in range(_rng.randint(per_hour // 2, per_hour)):
            seq += 1
            # ~5% late events: the business event happened hours before the API recorded it
            if _rng.random() < 0.05:
            ...
            _EVENTS.append(
                {
                    ...
                    "member_id": f"M{_rng.randint(100_000, 199_999)}",
...
def get_events(
    since: str = Query(..., description="ISO timestamp; returns events with event_time >= since"),
