...
Deliberate real-world quirks baked in:
  * ~2% duplicate rows (same claim_id appears twice)   -> tests idempotency
  * ~5% late-arriving rows (fill_time up to 6h before file hour) -> tests watermarks
  * Files on/after SCHEMA_CHANGE_DATE gain a new column `pharmacy_npi` -> tests schema evolution

Usage:
    python generate_hourly_csv.py --hours 24                 # last 24 hours
    python generate_hourly_csv.py --start 2026-07-01T00:00 --hours 48   # backfill
"""
...
SCHEMA_CHANGE_DATE = datetime(2026, 7, 3)  # files after this date have an extra column
ROWS_PER_FILE = 500
DUPLICATE_RATE = 0.02
LATE_RATE = 0.05
...
def make_row(file_hour: datetime, rng: random.Random) -> dict:
    """Build one pharmacy claim row anchored to a given file hour."""
    ndc, drug_name, base_cost = rng.choice(DRUGS)

    # Most fills happen within the file's hour; a few arrive late (older event time)
    ...
    return {
        ...
        "member_id": f"M{rng.randint(100_000, 199_999)}",
        "ndc_code": ndc,
        "drug_name": drug_name,
...
def main() -> None:
    ...
    parser.add_argument("--hours", type=int, default=24, help="number of hourly files")
    parser.add_argument("--start", type=str, default=None, help="start hour ISO, e.g. 2026-07-01T00:00")
